...
class CreateManifestOrderlinesTable extends Migration {
	...
	public function up()
	{
		Schema::create('manifest_orderlines', function(Blueprint $table)
		{
			$table->increments('id');
			$table->integer('ext_id')->nullable()->index('ext_id');
			$table->string('uid')->nullable();
			$table->string('metrc_tag')->nullable();
			$table->string('metrc_package_created')->nullable();
			$table->decimal('metrc_quantity', 10)->nullable();
			$table->string('metrc_uom')->nullable();
	...
	{
		Schema::drop('manifest_orderlines');
	}
